...
class TestCaller:
    ...
    def test_make_request_params(self, client, model, prompt):
        ...
        caller = Caller(client=client, model=model, prompt=prompt, handler=Mock(), request_params=params)
        assert caller.client == client
        assert caller.model == model
        assert caller.prompt == prompt
    ...
    def test_make_request_params_with_model(self, client, model, prompt):
        with pytest.raises(ValueError):
            Caller(client=client, model=model, prompt=prompt, handler=Mock(), request_params={"model": model})

    def test_signature(self, client, model, prompt, passthrough_handler):
        caller = Caller(client=client, model=model, prompt=prompt, handler=passthrough_handler)
    ...
    def test_handle_with_repair_success(self, client, model, prompt, chat_response):
        """Test successful handling without needing repair."""
        mock_handler = Mock()
        mock_handler.return_value = AssistantMessage(content="Success!")

        caller = Caller(client=client, model=model, prompt=prompt, handler=mock_handler)
        ...
        mock_handler.assert_called_once_with(chat_response)

    def test_handle_with_repair_retry_success(self, client, model, prompt, chat_response):
        """Test repair with successful retry."""
        mock_handler = Mock()
        mock_handler.side_effect = [
            ValueError("Bad response"),  # First call fails
            AssistantMessage(content="Fixed!"),  # Second call succeeds
        ]
        ...
        assert len(conversation.messages) == 4  # Repair messages added
    ...
    def test_handle_with_repair_max_attempts(self, client, model, prompt, chat_response):
        """Test reaching max repair attempts."""
        mock_handler = Mock(side_effect=ValueError("Bad response"))
        mock_handler.repair = Mock(
            return_value=Conversation(
                messages=[
                    AssistantMessage(content="invalid"),
        ...
        assert mock_handler.call_count == caller.max_repair_attempts + 1

    def test_handle_with_repair_no_instructions(self, client, model, prompt, chat_response):
        """Test repair failing due to no repair instructions."""
        mock_handler = Mock(side_effect=ValueError("Bad response"))
        mock_handler.repair = Mock(return_value=None)  # No repair instructions
...
class TestCallerHandlers:
    ...
    def test_with_pydantic_handler(self, client, model, prompt, pydantic_handler, structured_response):
        ...
        result = caller(user_vars={"topic": "test input"})
        assert isinstance(result, AssistantMessage)
        assert result.content == json.dumps({"name": "Bob", "age": 42})
    ...
    def test_with_tool_handler(self, client, model, prompt, tool_handler, tool_response):
        ...
        caller = Caller(client=client, model=model, prompt=prompt, handler=tool_handler)

        result = caller(user_vars={"topic": "test input"})
        assert isinstance(result, AssistantMessage)
        assert result.content == json.dumps({"x": 1, "y": 2})
    ...
    # CompositeHandler should work just fine given it passes handler tests
...
class TestCallerHelpers:
    ...
    def test_make_tool_params_openai(self, add_tool):
        ...
        # Add to TestCallerHandlers class in tests/core/test_caller.py
